...
var reList = new RegExp('^\/information$','i');
var reItem = new RegExp('^\/information\/.*','i');
...
function postHtmlItem(req, res) {
    ...
    req.on('end', function() {
        try {
            ...
            res.writeHead(303,'See Other', {'Location' : root+'/information/'+item.id});
            res.end();
        }
        catch (ex) {
            sendHtmlError(req, res, 'Server Error', 500);
...
function updateAPIItem(req, res, id) {
    ...
    req.on('end', function() {
        try {
            msg = JSON.parse(body);
            item = messages('update', id, {name:msg.template.data[0].value,
                                           studentid:msg.template.data[1].value,
                                           email:msg.template.data[2].value,
                                           phone:msg.template.data[3].value,
                                           hobby:msg.template.data[4].value}).item;
...
function postAPIItem(req, res) {
    ...
    req.on('end', function() {
        try {
            msg = JSON.parse(body);
            item = messages('add', {name:msg.template.data[0].value,
                                    studentid:msg.template.data[1].value,
                                    email:msg.template.data[2].value,
                                    phone:msg.template.data[3].value,
                                    hobby:msg.template.data[4].value}).item;
...
function formatHtmlItem(item) {
    ...
    rtn += '<dt>STUDENTID</dt><dd>'+item.studentid+'</dd>';
    rtn += '<dt>EMAIL</dt><dd>'+item.email+'</dd>';
    rtn += '<dt>PHONE</dt><dd>'+item.phone+'</dd>';
    rtn += '<dt>HOBBY</dt><dd>'+item.hobby+'</dd>';
    rtn += '</dl>\n';
...
function formatHtmlList(list) {
    ...
    for(i=0,x=list.length;i<x;i++) {
        rtn += '<li>';
        rtn += '<a href="'+root+'/information/'+list[i].id+'" title="' + list[i].date+'">';
        rtn += list[i].name;
        rtn += list[i].studentid;
        rtn += list[i].email;
        rtn += list[i].phone;
        rtn += list[i].hobby;
        rtn += '</a></li>\n';
...
function formatAPIList(list) {
    ...
    for(i=0,x=list.length; i<x; i++) {
        ...
        item.data.push({name:"phone", value:list[i].phone});
...
function formatAPIItem(item) {
    ...
    rtn.data.push({name:"studentid", value:item.studentid});
    rtn.data.push({name:"email", value:item.email});
    rtn.data.push({name:"phone", value:item.phone});
